...
1. Read **`lessons://overview`** from the Lessons Learned MCP server (optional: `lessons://search-guide`).
...
4. For **project-specific** paths, env, or conventions: use the Project Details MCP server (`/mcp/project-details?project=<source>`). The `project` param must match `php artisan mcp:push --source=…`. Call **GetProjectDetailsOverview** when connected.
5. **Apply lessons** to coding decisions; query again when new topics arise.
6. Call **MarkLessonHelpful** when a lesson clearly helped or hurt (feeds relevance scoring and dashboard helpfulness rate).
