...
if [[ ! -f "${PROMPT_FILE}" ]]; then
  PROMPT_FILE="${SCRIPT_DIR}/../../packages/laravel-mcp-pusher/stubs/knowledge-capture-prompt.txt"
fi

if [[ ! -f "${PROMPT_FILE}" ]]; then
  echo '{"user_message":"Context is about to compact. Open packages/laravel-mcp-pusher/stubs/knowledge-capture-prompt.txt and paste it to your agent."}'
  exit 0
fi
